...
var wsUri = 'https://' + location.host + '/monoliticsfu';
var videoFeed;
var webRtcPeer;
var stream;
var newStreams = 0
...
function onAddStream (mediaStream) {
	var stream = mediaStream.stream
	var name = "video_" + nStreams;
	document.getElementById('remoteStreams').innerHTML += '<video id=\"'+ name + '\" autoplay width=\"480px\" height=\"360px\" poster=\"img/webrtc.png\"></video><br>';
...
//	originStream = this.peerConnection.getLocalStreams()[0].clone();
	...
	jsonrpcClient.send('negotiateWebRtc', {sdpOffer: sdpOffer}, function(error, answer) { 
		...
		webRtcPeer.processAnswer(answer.sdpAnswer, function(error) {
			...
			jsonrpcClient.send('connectUsers', {}, function(error) {
				if (error) return setState(I_CAN_START)
			});
		});
	});
...
function viewerNegotiation(remoteSdpOffer) {
	console.log ("Renegotiate sdp offer from server \r\n ", remoteSdpOffer);
